...
const activeFactors = computed(() => {
  ...
  const pot = factorLabel('pot', f.potVolumeFactor)
  if (pot) items.push({ key: 'pot', label: pot })
  const potMaterial = factorLabel('potMaterial', f.potMaterialFactor)
  if (potMaterial) items.push({ key: 'potMaterial', label: potMaterial })
  ...
  const humidity = factorLabel('humidity', f.humidityFactor)
  if (humidity) items.push({ key: 'humidity', label: humidity })
